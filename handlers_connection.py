"""Connection lifecycle for Tealium Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from tealium_client import TealiumClient
...
@chat.function(
    "connect_tealium",
    "Connect Tealium account via credentials.",
    action_type="write",
    chain_callable=True,
    event="tealium-connector.connect_tealium",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_tealium(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Tealium Connector."""
    client = TealiumClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Tealium Account",
...
@chat.function(
    "list_connections",
    "List connected Tealium accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_tealium",
    "Disconnect Tealium account.",
    action_type="write",
    chain_callable=True,
    event="tealium-connector.disconnect_tealium",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_tealium(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
